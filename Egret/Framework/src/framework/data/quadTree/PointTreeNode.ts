 ...
 * 点四叉树节点
 */
class PointTreeNode<T extends IQuadTreeItem> {

    public rect:QuadTreeRect;
    public depth:number;
    public maxDepth:number;
    public maxChildren:number;
    public n1:PointTreeNode<T>;
    public n2:PointTreeNode<T>;
    public n3:PointTreeNode<T>;
    public n4:PointTreeNode<T>;
    ...
    public insert(item:T):void {
        if (this.divided) {
            this.findNode(item.bounds).insert(item);
            return;
        }
    ...
    public retrieve(bounds:QuadTreeBounds, outList:Array<T>):Array<T> {
        if (this.divided) {
            var node:PointTreeNode<T> = this.findNode(bounds);
     ...
     * 查找索引
     */
    public findNode(bounds:QuadTreeBounds):PointTreeNode<T> {
    ...
    protected subdivide():void {
        ...
        this.n1 = new PointTreeNode<T>(new QuadTreeRect(bx, by, b_w_h, b_h_h), depth, this.maxDepth, this.maxChildren);
        this.n2 = new PointTreeNode<T>(new QuadTreeRect(bx_b_w_h, by, b_w_h, b_h_h), depth, this.maxDepth, this.maxChildren);
        this.n3 = new PointTreeNode<T>(new QuadTreeRect(bx, by_b_h_h, b_w_h, b_h_h), depth, this.maxDepth, this.maxChildren);
        this.n4 = new PointTreeNode<T>(new QuadTreeRect(bx_b_w_h, by_b_h_h, b_w_h, b_h_h), depth, this.maxDepth, this.maxChildren);
